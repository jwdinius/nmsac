...
bool nmsac::registration(arma::mat const & src_sub, arma::mat const & tgt_sub,
    ...
    cor::qap::Config reg_config;  // TODO(jwd) - need a better abstraction for sharing config data
    ...
    std::unique_ptr<cor::CorrespondencesBase> qap = std::make_unique<cor::QAP>(src_sub, tgt_sub, reg_config);
    auto const corr_status = qap->calc_correspondences(corrs);

    // TODO(jwd) - check for status and exit on failure
